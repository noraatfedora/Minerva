    ...
    if request.method == "POST" and 'num_vehicles' in request.values.to_dict().keys():
        ...
        return loadingScreen(num_vehicles=int(request.values.get('num_vehicles')),
                             global_span_cost=int(
                                 request.values.get('global_span_cost')),
                             stop_conversion=int(
                                 request.values.get('stop_conversion')),
                             solution_limit=int(
                                 request.values.get('solution_limit')),
        ...
        clients = loads(conn.execute(select([routes.c.content]).where(
...
def loading_status(foodBankId):
    status = conn.execute(select([users.c.doneCalculating]).where(
        users.c.id == foodBankId)).fetchone()[0]
    return str(status)
...
def google_maps_redirect(query):
    ...
    foodBank = conn.execute(users.select().where(
        users.c.apiKey == api_key)).fetchall()
    if foodBank == None:
        return "Sorry, that URL contains an invalid API key."
    userList = []
    for id in userIdList:
        userList.append(conn.execute(
...
def getRoutes():
    ...
    rp = conn.execute(routes.select().where(
        routes.c.foodBankId == g.user.id)).fetchall()
    if len(rp) == 0:
        return []
    scoreAverage = 0
    toReturn = []
...
def driver_printout(routeId):
    ...
    route['qr'] = google_maps_qr.make_qr_code(
        route['usersList'], g.user.apiKey)
    route['headerText'] = "Route " + str(routeId)
    route['length'] = conn.execute(select([routes.c.length]).where(
        routes.c.id == routeId)).fetchone()[0] / 1000.0
...
def master_driver_printout():
    ...
    for route in routesList:
        ...
        for user in routeDict['usersList']:
            qr_data = google_maps_qr.make_user_qr(user['formattedAddress'])
            user['qr_data'] = qr_data
        routeDict['qr'] = google_maps_qr.make_qr_code(
            routeDict['usersList'], g.user.apiKey)
        # + "(ID: " + str(route['id']) + ")"
...
def splitRoute(routeId):
    oldRoute = loads(conn.execute(select([routes.c.content]).where(
        routes.c.id == routeId)).fetchone()[0])
    print("Old route: " + str(oldRoute))
    newRouteRight = getUsersFromIdList(
        [g.user.id] + oldRoute[int(len(oldRoute)/2):])
    newRouteLeft = getUsersFromIdList(
        oldRoute[:int(len(oldRoute)/2)] + [g.user.id])
    ...
    conn.execute(routes.insert().values(content=dumps(
        leftContents), length=leftDistance, foodBankId=g.user.id))
    conn.execute(routes.insert().values(content=dumps(
        rightContents), length=rightDistance, foodBankId=g.user.id))
...
def calculateLength(route):  # requres list of user objects, like from getUsersFromIdList()
    ...
    for i in range(0, len(route) - 1):
        userFrom = route[i]
        userTo = route[i+1]
        distance += assign.measure(userFrom.latitude,
                                   userFrom.longitude, userTo.latitude, userTo.longitude)
    return distance
